## To prepare the gene expression profiles of TCGA samples
...
TCGA_RNASeq.BRCA.log2FPKM_isoformLevel.eset <- brca
save(TCGA_RNASeq.BRCA.log2FPKM_isoformLevel.eset, file = "TCGA_RNASeq.BRCA.log2FPKM_isoformLevel.eset")
...
draw.eset.QC(TCGA_RNASeq.BRCA.log2FPKM_geneLevel.eset, outdir = "./", do.logtransform = FALSE, prefix = 'TCGA_RNASeq.BRCA.log2FPKM_geneLevel.',
...
#################### FPKM ####################
## 1. Load the TCGA dataset
load("TCGA.Oncoland.201503.count.eset") # curated from Qiagen Oncoland TCGA dataset
eset <- TCGA.Oncoland.201503.count.eset; rm(TCGA.Oncoland.201503.count.eset)
...
#exprs(brca) <- RNASeqCount.normalize.scale(mat = as.matrix(exprs(brca)), total = 1000000, pseudoCount = 0)
#exprs(brca) <- log2(d + 1)

## 4. Save the eset of isoform level
d <- RNASeqCount.normalize.scale(mat = as.matrix(exprs(brca)), total = 1000000, pseudoCount = 0)
d <- log2(d + 1)
colnames(d) <- gsub("^X", "",colnames(d))
d <- d[, row.names(pData(brca))]
brca <- new("ExpressionSet", phenoData = new("AnnotatedDataFrame", pData(brca)), featureData=new("AnnotatedDataFrame", fData(brca)), annotation='MicroArray', exprs=as.matrix(d))
TCGA_RNASeq.BRCA.log2CPM_isoformLevel.eset <- brca
save(TCGA_RNASeq.BRCA.log2CPM_isoformLevel.eset, file = "TCGA_RNASeq.BRCA.log2CPM_isoformLevel.eset")
...
exp <- exp[row.names(fd),]; exp <- exp[, row.names(pd)] ## reorder the rows and columes of the expression matrix
TCGA_RNASeq.BRCA.log2CPM_geneLevel.eset <- new("ExpressionSet", phenoData = new("AnnotatedDataFrame", pd), featureData=new("AnnotatedDataFrame", fd), annotation='TCGA BRCA', exprs=as.matrix(exp))
save(TCGA_RNASeq.BRCA.log2CPM_geneLevel.eset, file = "TCGA_RNASeq.BRCA.log2CPM_geneLevel.eset")

draw.eset.QC(TCGA_RNASeq.BRCA.log2CPM_geneLevel.eset, outdir = "./", do.logtransform = FALSE, prefix = 'TCGA_RNASeq.BRCA.log2CPM_geneLevel.',
             intgroup = NULL, choose_plot = c("heatmap", "pca", "density", "correlation", "meansd"), generate_html = TRUE, correlation_strategy = "pearson", plot_all_point = FALSE,
             emb_plot_type='2D.ellipse' # "2D", "2D.interactive", "2D.ellipse", "2D.text" or "3D" 
)
