## To calculate the HDAC6 score of Merged Human BRCA samples
...
## 1. load the eset
load("FourSets.TCGA_METABRIC_IBC_CLINICAL.afterBatchEffectRemoval.eset")
eset <- FourSets.TCGA_METABRIC_IBC_CLINICAL.afterBatchEffectRemoval.eset; rm(FourSets.TCGA_METABRIC_IBC_CLINICAL.afterBatchEffectRemoval.eset)
...
eset.act <- generate.eset(exp_mat = d, phenotype_info = pd, feature_info = fd, annotation_info = 'MergedHumanBRCA, GeneLevel, Activity')
MergedHumanBRCA.activity.eset <- eset.act
save(MergedHumanBRCA.activity.eset, file = "MergedHumanBRCA.activity.eset")
