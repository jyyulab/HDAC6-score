...
eset <- METABRIC_microArray.BRCA.log2Intensity_geneLevel.eset
...
for (i in 1:length(type)) {
  ...
  expMatrix.output <- data.frame(isoformId = fd.filtered$GeneID, geneSymbol = fd.filtered$GeneID, expMatrix.filtered)
  
  dir1 <- paste0(type[i], "/log2Intensity"); dir.create(dir1, recursive = T)
